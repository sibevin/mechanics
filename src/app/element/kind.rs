...
fn refresh_elements(
    ...
    if build_timer.0.tick(time.delta()).just_finished() {
        ...
        for (g_trans, _, _, ele_data) in ele_query.iter() {
            match ele_data {
                ElementData::Slider {
                    data: _,
                    is_modifier_on: _,
...
fn handle_element_keyboard_pressing(
    ...
    let mut press_action = None;
    if input.any_just_pressed([KeyCode::Space, KeyCode::Enter]) {
        press_action = Some(ElementAction::Confirm);
    }
...
fn handle_element_keyboard_changing(
    ...
    let mut is_triggered: bool = false;
    if kb_input.any_pressed([KeyCode::ArrowRight, KeyCode::KeyD]) {
    ...
    if kb_input.any_pressed([KeyCode::ArrowUp, KeyCode::KeyW]) {
        key_actions.push(ElementAction::Up);
    }
    if kb_input.any_pressed([KeyCode::ArrowDown, KeyCode::KeyS]) {
        key_actions.push(ElementAction::Down);
    }
    if kb_input.any_just_pressed([KeyCode::ArrowRight, KeyCode::KeyD]) {
        is_triggered = true;
    }
    if kb_input.any_just_pressed([KeyCode::ArrowLeft, KeyCode::KeyA]) {
        is_triggered = true;
    }
    if kb_input.any_just_pressed([KeyCode::ArrowUp, KeyCode::KeyW]) {
        is_triggered = true;
    }
    if kb_input.any_just_pressed([KeyCode::ArrowDown, KeyCode::KeyS]) {
        is_triggered = true;
    }
