...
const SHOOT_W: f32 = ui::FONT_SIZE * 0.06;
...
pub fn handle_menu_entry_interaction(
    ...
    if let Some(target) = target {
        let circle_target: FocusTarget = FocusTarget {
            pos: Vec2::new(
                target.pos.x - target.size.x * 0.5 + target.size.y * 0.4,
                target.pos.y,
            ),
...
const SLIDER_P: f32 = ui::FONT_SIZE;
